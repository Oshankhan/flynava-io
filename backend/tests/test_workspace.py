...
def test_org_me_employee(client, auth_header):
    r = client.get("/api/v1/org/me", headers=auth_header("employee@flynava.ai"))
    assert r.status_code == 200
    body = r.json()
    assert body["level"] == 1
    assert body["team"]["team_id"] == "team_python"
    assert body["lead"]["user_id"] == "u_tl_python"
    assert body["reports"] == []


def test_org_me_team_lead_has_reports(client, auth_header):
    r = client.get("/api/v1/org/me", headers=auth_header("python.tl@flynava.ai"))
    body = r.json()
    assert body["level"] == 2
    assert any(m["user_id"] == "u_emp" for m in body["reports"])


def test_auth_me_carries_hierarchy(client, auth_header):
    r = client.get("/api/v1/auth/me", headers=auth_header("employee@flynava.ai"))
    u = r.json()["user"]
    assert u["level"] == 1
    assert u["team_id"] == "team_python"
    assert u["reports_to"] == "u_tl_python"
    assert u["designation"] == "Python Developer"


# --- Tasks ---

def test_my_tasks_buckets(client, auth_header):
    r = client.get("/api/v1/tasks/my", headers=auth_header("employee@flynava.ai"))
    assert r.status_code == 200
    body = r.json()
    # seeded: t1 Done, t2 In progress + overdue for u_emp
    assert body["buckets"]["total"] == 2
    assert body["buckets"]["completed"] == 1
    assert body["buckets"]["overdue"] == 1


def test_create_task_self_and_forbidden_other(client, auth_header):
    h = auth_header("employee@flynava.ai")
    ok = client.post("/api/v1/tasks", headers=h,
                     json={"title": "Write unit tests"})
    assert ok.status_code == 200
    assert ok.json()["assignee_id"] == "u_emp"
    denied = client.post("/api/v1/tasks", headers=h,
                         json={"title": "Nope", "assignee_id": "u_qa1"})
    assert denied.status_code == 403


def test_lead_assigns_task_to_report_and_notifies(client, auth_header, db):
    h = auth_header("python.tl@flynava.ai")
    r = client.post("/api/v1/tasks", headers=h,
                    json={"title": "Review PR", "assignee_id": "u_emp"})
    assert r.status_code == 200
    assert db.notifications.find_one({"recipient_id": "u_emp",
                                      "type": "task_assigned"})
    assert db.audit_logs.find_one({"action": "task_created",
                                   "actor_id": "u_tl_python"})


def test_team_tasks_requires_lead(client, auth_header):
    assert client.get("/api/v1/tasks/team",
                      headers=auth_header("employee@flynava.ai")).status_code == 403
    r = client.get("/api/v1/tasks/team", headers=auth_header("python.tl@flynava.ai"))
    assert r.status_code == 200
    assert any(m["user_id"] == "u_emp" for m in r.json()["members"])
...
def test_meeting_create_list_cancel(client, auth_header, db):
    ...
    assert client.delete(f"/api/v1/meetings/{mid}",
                         headers=auth_header("employee@flynava.ai")).status_code == 404
    assert client.delete(f"/api/v1/meetings/{mid}", headers=h).status_code == 200


def test_seeded_meetings_visible(client, auth_header):
    r = client.get("/api/v1/meetings/my", headers=auth_header("employee@flynava.ai"))
...
def test_request_routes_to_direct_lead_and_approve(client, auth_header, db):
    ...
    assert r.status_code == 200
    req = r.json()
    assert req["approver_id"] == "u_tl_python"  # direct lead first
    assert db.notifications.find_one({"recipient_id": "u_tl_python",
...
def test_request_forward_moves_up_chain(client, auth_header):
    ...
    assert fwd.json()["approver_id"] == "u_mgr"  # TL's own lead = eng head

    # engineering head can now approve
    h_head = auth_header("manager@flynava.ai")
...
def test_stranger_cannot_decide(client, auth_header):
    ...
    denied = client.post(f"/api/v1/requests/{req['req_id']}/approve",
                         headers=auth_header("qa.tl@flynava.ai"), json={})
    assert denied.status_code == 404


def test_bad_request_type_rejected(client, auth_header):
    r = client.post("/api/v1/requests", headers=auth_header("employee@flynava.ai"),
...
def test_workspace_me_shape(client, auth_header):
    r = client.get("/api/v1/workspace/me", headers=auth_header("employee@flynava.ai"))
    assert r.status_code == 200
    body = r.json()
    assert body["user"]["name"] == "Evan Employee"
    assert body["user"]["team_id"] == "team_python"
    assert body["team"]["name"] == "Python Team"
    assert body["lead"]["name"] == "Priya Nair"
...
def test_activity_scoped_to_self_for_l1(client, auth_header):
    h_emp = auth_header("employee@flynava.ai")
    h_qa = auth_header("qa.eng@flynava.ai")
    ...
    assert all(a["actor_id"] == "u_emp" for a in acts)


def test_audit_middleware_records_mutations(client, auth_header, db):
    client.post("/api/v1/tasks", headers=auth_header("employee@flynava.ai"),
                json={"title": "Audit me"})
