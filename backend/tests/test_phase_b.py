...
def test_authored_matches_openproject_author_field(client, auth_header, db):
    db.tasks.insert_one({
        ...
        "assignee": "Someone Else", "author": "Evan Employee",
...
def test_compliance_items_readable_by_any_user(client, auth_header):
    r = client.get("/api/v1/compliance/items", headers=auth_header("employee@flynava.ai"))
    assert r.status_code == 200
    items = r.json()
    assert len(items) == 3
...
def test_positions_open_and_dept_filter(client, auth_header):
    r = client.get("/api/v1/positions", headers=auth_header("recruiter@flynava.ai"))
    assert r.status_code == 200
    assert len(r.json()) == 3
    eng_only = client.get("/api/v1/positions?dept=eng",
                          headers=auth_header("recruiter@flynava.ai")).json()
...
def test_kpi_history_department_scoped_for_l1_and_l2(client, auth_header):
    ...
    ok = client.get("/api/v1/kpis/mkt_leads/history",
                    headers=auth_header("marketing.exec@flynava.ai"))
    ...
    ok2 = client.get("/api/v1/kpis/mkt_conversion/history",
                     headers=auth_header("marketing.tl@flynava.ai"))
    assert ok2.status_code == 200
    ok = client.get("/api/v1/kpis/fin_revenue_mtd/history",
                    headers=auth_header("finance.tl@flynava.ai"))
    assert ok.status_code == 200
    body = ok.json()
...
def test_pending_leaves_scoping(client, auth_header):
    denied = client.get("/api/v1/hr/leaves", headers=auth_header("employee@flynava.ai"))
    assert denied.status_code == 403
    # HR team lead (department=hr) is allowed even though role is team_lead
    ok = client.get("/api/v1/hr/leaves", headers=auth_header("hr.tl@flynava.ai"))
    assert ok.status_code == 200
    rows = ok.json()
    assert len(rows) > 0
    ...
    denied2 = client.get("/api/v1/hr/leaves", headers=auth_header("python.tl@flynava.ai"))
...
def test_pending_leave_approve_decrements_balance(client, auth_header, db):
    ...
    h = auth_header("hr.tl@flynava.ai")
...
def test_reports_projects_allows_team_lead(client, auth_header):
    r = client.get("/api/v1/reports/projects", headers=auth_header("qa.tl@flynava.ai"))
    assert r.status_code == 200
    denied = client.get("/api/v1/reports/projects", headers=auth_header("qa.eng@flynava.ai"))
    assert denied.status_code == 403
